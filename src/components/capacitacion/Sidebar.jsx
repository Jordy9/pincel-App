    ...
    <div style={{width: '100%', height: '80vh'}}>
        <h4 className='text-center p-1'>Contenido de la capacitacion</h4>
        {
            capacitacionFiltrada[0]?.video?.map((capacitacion, index) => {
                ...
                return (
                    <div onClick={() => dispatch(activeCapacitacion({_id: capacitacionFiltrada[0]?._id, videos: capacitacion, preguntas: capacitacionFiltrada[0]?.Preguntas, descripcion: capacitacionFiltrada[0]?.descripcion, usuariosEvaluacion: capacitacionFiltrada[0]?.usuariosEvaluacion, intentos: capacitacionFiltrada[0]?.intentos, EvaluatShow: capacitacionFiltrada[0]?.EvaluatShow}))} className={`row p-2 sidebarCapacitacion ${(capacitacionActiva?.videos === capacitacion) && 'sidebarCapacitacionfocus'}`} key={capacitacion?._id}>
